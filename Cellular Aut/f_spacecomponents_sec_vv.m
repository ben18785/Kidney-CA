function [v_xposs_princ,v_yposs_princ] = f_spacecomponents_sec_vv(v_delta_move,c_xnew,c_ynew,v_parameters,c_prin_or_sec)
...
if abs(v_delta_move(1)) > 0 % Only if there is some x movement in the principal direction
    for i = 1:2*c_prin_or_sec + 1
     v_xposs_princ(i) = c_xnew - c_prin_or_sec*v_delta_move(1) + (i-1)*v_delta_move(1);
    end
end
...
if abs(v_delta_move(2)) > 0 % Only if there is some y movement in the principal direction
     for i = 1:2*c_prin_or_sec + 1
     v_yposs_princ(i) = mod(c_xnew - c_prin_or_sec*v_delta_move(1) + (i-1)*v_delta_move(1) - 1,c_width_full);
    end
end