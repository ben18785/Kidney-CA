function [c_allowed,m_allowedindices] = f_epithelium_mrule8_cm(c_x,c_y,m_allindices,m_cell,v_parameters,cp_move)
% A function which finds only those indices which allows moves if the
% active (moving) cell is not going to be unconnected, and there are
% available cells in the 'vicinity' for the mesenchyme to move into. 
...
for i = 1:cd_indicesmax
    ...
    switch cp_move
        case 1 % Moving
            ...
            elseif m_cell(m_allindices(i,1),m_allindices(i,2))== - 1 % If a mesenchyme, allow a movement iff there are vacant spots in the vicinity for the mesenchyme to move into
                [c_allowed,~] = f_allindices_allnearby_m(c_x,c_y,m_allindices(i,1),m_allindices(i,2),m_cell,v_parameters); % Find all possible available indices nearby
