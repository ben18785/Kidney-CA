function m_cell = f_pprolifselector_m(c_x,c_y,m_allowedindices,m_cell,m_GDNF,v_parameters)
...
switch ck_prolif_choosecell_rule
    case 1 % All moves are given equal probability
        m_cell = f_pprolif_rule1_m(c_x,c_y,m_allowedindices,m_cell,m_GDNF,v_parameters);
    case 2 % Move probability is weighted by the magnitude of the gradient of GDNF
        m_cell = f_pprolif_rule2_m(c_x,c_y,m_allowedindices,m_cell,m_GDNF,v_parameters);
    case 3 % Move probability is weighted by the percentage increase in GDNF available
        m_cell = f_pprolif_rule3_m(c_x,c_y,m_allowedindices,m_cell,m_GDNF,v_parameters);
    case 4 % Same as case 1 but now the mesenchyme are moved if the epithelium wants to move into their spot
        if ck_movement_rule == 6
            m_cell = f_pprolif_rule4_m(c_x,c_y,m_allowedindices,m_cell,m_GDNF,v_parameters);
        else
